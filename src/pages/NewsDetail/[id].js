...
const NewsDetail = () => {
  ...
  console.log(router.query.id, " ids");
  const matchingId = router.query.id;
  const { newsData } = useContext(AuthContext);

  const getCustomData = (id) => {
    const currentData = newsData.filter((item) => item.id == id);
    const json = currentData.pop();
    return json;
  };
  const obj = getCustomData(matchingId);
